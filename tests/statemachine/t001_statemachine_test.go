...
type t18_state_transitions struct {
	statemachine.StateMachine[t18_state_transitions_data]
}
...
func (sm *t18_state_transitions) Init(args ...any) (statemachine.StateMachineSpec[t18_state_transitions_data], error) {
	spec := statemachine.NewStateMachineSpec(gen.Atom("state1"),
		// initial data
		statemachine.WithData(t18_state_transitions_data{}),

		// set up a message handler for the transition [state1] -> [state2]
		statemachine.WithStateMessageHandler(gen.Atom("state1"), t18_handle_state2),

		// set up a call handler to query the number of state transitions
		statemachine.WithStateCallHandler(gen.Atom("state2"), t18_handle_get_state_transitions),
	)

	return spec, nil
}

func t18_handle_state2(state gen.Atom, data t18_state_transitions_data, message t18_state2, proc gen.Process) (gen.Atom, t18_state_transitions_data, []statemachine.Action, error) {
	data.transitions++
	return gen.Atom("state2"), data, nil, nil
}

func t18_handle_get_state_transitions(state gen.Atom, data t18_state_transitions_data, message t18_get_state_transitions, proc gen.Process) (gen.Atom, t18_state_transitions_data, int, []statemachine.Action, error) {
