...
type t19_events struct {
	statemachine.StateMachine[t19_events_data]
	tc *testcase
}
...
func (sm *t19_events) Init(args ...any) (statemachine.StateMachineSpec[t19_events_data], error) {
	spec := statemachine.NewStateMachineSpec(gen.Atom("state1"),
		// initial data
		statemachine.WithData(t19_events_data{}),

		// register event handler
		statemachine.WithEventHandler(gen.Event{Name: "testEvent", Node: "t19node@localhost"}, t19_handle_test_event),

		// register call handler
		statemachine.WithStateCallHandler(gen.Atom("state1"), t19_handle_get_events_received),
	)

	return spec, nil
}

func t19_handle_get_events_received(state gen.Atom, data t19_events_data, event t19_get_events_received, proc gen.Process) (gen.Atom, t19_events_data, int, []statemachine.Action, error) {
	return state, data, data.eventsReceived, nil, nil
}

func t19_handle_test_event(state gen.Atom, data t19_events_data, event t19_event, proc gen.Process) (gen.Atom, t19_events_data, []statemachine.Action, error) {
	data.eventsReceived++
