...
type t20_state_enter_callback struct {
	statemachine.StateMachine[t20_state_enter_callback_data]
	tc *testcase
}
...
func (sm *t20_state_enter_callback) Init(args ...any) (statemachine.StateMachineSpec[t20_state_enter_callback_data], error) {
	spec := statemachine.NewStateMachineSpec(gen.Atom("state1"),
		// initial data
		statemachine.WithData(t20_state_enter_callback_data{currentState: gen.Atom("state1")}),

		// with message handler for [state1] -> [state2]
		statemachine.WithStateMessageHandler(gen.Atom("state1"), t20_handle_state2),

		// with call handler to query the data
		statemachine.WithStateCallHandler(gen.Atom("state1"), t20_state_and_callback_count),
		statemachine.WithStateCallHandler(gen.Atom("state3"), t20_state_and_callback_count),

		// with state enter callback
		statemachine.WithStateEnterCallback(t20_state_enter),
	)

	return spec, nil
}

func t20_handle_state2(state gen.Atom, data t20_state_enter_callback_data, message t20_state2, proc gen.Process) (gen.Atom, t20_state_enter_callback_data, []statemachine.Action, error) {
	return gen.Atom("state2"), data, nil, nil
}

func t20_state_and_callback_count(state gen.Atom, data t20_state_enter_callback_data, message t20_state_enter_callback_query, proc gen.Process) (gen.Atom, t20_state_enter_callback_data, t20_state_enter_callback_data, []statemachine.Action, error) {
