...
func TestTopNActorInitNormalizesZeroIntervalAndTopN(t *testing.T) {
	shared := NewShared()
	ta, err := startTopNNode(t).Spawn(TopNActorFactory, gen.ProcessOptions{}, TopNActorOptions{
		Name:   "test_zero_options",
		Help:   "test help",
		Labels: []string{"pid", "name"},
		Order:  TopNMax,
		Shared: shared,
		Owner:  makeOwnerPID(),
		// TopN and Interval left at zero: a caller that omits them, which is
		// what radar's own options did until they were normalized there too.
	})
	if err != nil {
		t.Fatalf("failed to spawn topN actor: %v", err)
	}

	// A zero interval is not "no delay", it is a flush due immediately - and
	// since every flush re-arms, the actor would spin without ever yielding.
	ta.ShouldSendAfter().
		Message(messageFlush{}).
		Where(func(r check.SendAfter) bool { return r.After == DefaultCollectInterval }).
		Once().
		Must()

	// A zero topN makes the heap discard every observation, so the metric
	// would register and then never report anything.
	sender := gen.PID{Node: "test@localhost", ID: 5000, Creation: 1}
	ta.SendMessage(sender, MessageTopNObserve{Value: 100, Labels: []string{"pid1", "worker1"}})
	ta.SendMessage(sender, messageFlush{})
	...
	var samples int
	for _, mf := range families {
		if mf.GetName() == "test_zero_options" {
			samples = len(mf.GetMetric())
		}
	}
	if samples != 1 {
		t.Fatalf("expected the observation to be kept, got %d samples", samples)
	}
}

// --- timer-driven flush ---

func TestTopNActorTimerFiresFlushAndRearms(t *testing.T) {
	shared := NewShared()
	ta := spawnTopNActor(t, "test_timer_flush", 5, TopNMax, shared)
	...
	// The flush scheduled in Init is addressed to the actor itself, so the
	// harness delivers it - a flush aimed elsewhere would only be recorded.
	if fired := ta.FireTimers(); fired != 1 {
		t.Fatalf("expected the scheduled flush to be delivered, fired %d timers", fired)
	}
	...
	var samples int
	for _, mf := range families {
		if mf.GetName() == "test_timer_flush" {
			samples = len(mf.GetMetric())
		}
	}
	if samples != 1 {
		t.Fatalf("the timer did not flush: %d samples", samples)
	}

	// and the loop closed: the delivered flush scheduled the next one
	ta.ShouldSendAfter().Message(messageFlush{}).Since(mark).Once().Must()
}

// --- name registration ---

func TestTopNActorInitRegisterNameFails(t *testing.T) {
	shared := NewShared()

	ta := unit.Prepare(t, TopNActorFactory, gen.ProcessOptions{}, TopNActorOptions{
	...
	ta.OnRegisterName(func(name gen.Atom) error { return gen.ErrTaken })

	err := ta.Run()
	if err == nil {
		t.Fatal("expected Init to fail when the process name is taken")
	}
	check.ErrorIs(t, err, gen.ErrTaken)

	// the gauge must not be left in the registry after a failed Init
	families, gatherErr := shared.registry.Gather()
	if gatherErr != nil {
		t.Fatalf("failed to gather: %v", gatherErr)
	}
	for _, mf := range families {
		if mf.GetName() == "test_register_name" {
			t.Error("a failed Init left its gauge registered")
		}
	}
}
